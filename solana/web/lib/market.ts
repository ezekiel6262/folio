...
export type StockMarket = {
  symbol: string
  /** Market price of one token, USD. */
  tokenUsd: number
  /** Price of one share-equivalent: token price divided by the multiplier. */
  shareUsd: number
  multiplier: number
...
async function jupiterPrices(mints: string[]): Promise<Record<string, number>> {
    ...
    try {
      ...
      return Object.fromEntries(
        Object.entries(json)
          ...
          .map(([mint, v]) => [mint, Number(v.usdPrice)]),
      )
    } catch {
      // try the next base
    }
...
export async function getMarket(): Promise<Market> {
  ...
  const [prices, mults, fx] = await Promise.all([
  ...
  for (const s of STOCKS) {
    const tokenUsd = prices[s.mint]
    if (!tokenUsd) continue
    const { multiplier, scheduled } = mults[s.symbol] ?? { multiplier: 1 }
    stocks[s.symbol] = { symbol: s.symbol, tokenUsd, shareUsd: tokenUsd / multiplier, multiplier, scheduled }
  }
  ...
  for (const c of STABLECOINS) {
    ...
    stablecoinUsd[c.symbol] = prices[c.mint] ?? (c.pegged === 'USD' ? 1 : 1 / (fx[c.pegged] ?? NaN))
  }

  const value: Market = { stocks, stablecoinUsd, fx, asOf: Math.floor(Date.now() / 1000), source: 'jupiter' }
